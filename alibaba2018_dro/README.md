...
| `scheduler.py` | 统一使用 Gurobi 的确定性/SAA/静态 Γ-RO/TV-DRO 日前模型与有限批处理追索，三类运行风险和实际回放；碳割仅供历史诊断复现 |
...
确定性入口是 `scripts/run_four_windows.py`；SAA 校准入口是 `scripts/run_uncertainty_methods.py`；静态 Γ-RO 校准入口是 `scripts/run_gamma_ro.py`。日前主问题和所有验证追索统一使用 Gurobi，命令行不再允许选择 SCIP。Γ-RO 数据流为“校准表训练折 → 24 小时位置下偏分位 → 本地 MW 偏差 → Γ 支持函数 + 完整算力包络 → 日前计划 → 共同验证回放”。当前主线不循环碳预算，碳排放只在求解后核算。

运行环境：

- `scheduler.py` 需 `gurobi_env` 中的 Gurobi 13.0.x 及可用许可证；仅复现历史碳对偶割时还需 SciPy/HiGHS；
- 其余模块 numpy / 标准库即可。
