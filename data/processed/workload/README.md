...
9. `generate_workload.py --training-scenario-count N` 可生成独立 CSV 场景池，但当前 SAA 主流程通过 `data/processed/scenarios/saa_scenarios_manifest.json` 按需重建嵌套训练、验证和回放场景。两种形式都不能替代公共名义负荷，也不能由不同方法分别挑选。

## 训练与测试边界

训练场景用于经验分布和不确定集标定；`nominal_workload_30d.csv` 是所有方法共用的日前名义输入；与训练种子不重叠、且所有方法共享的算力回放池用于评价违反率。当前默认生成命令仍取 `training_scenario_count=0`，但 SAA manifest 已登记训练、验证和 100 条回放轨迹，并已接入有限追索原型；完整三折和正式方法比较尚未运行。只在名义负荷上比较不能验证 SAA/RO/DRO 的可靠性。

训练池、公共 30 天负荷和回放池都来自相同的 7 个完整源日。它们是文件和预注册种子隔离的同源样本，不是独立年份或独立月份的算力数据。因此，30 天负荷应称为“公共名义输入”，回放称为“同源、种子隔离的重采样评价”；不能称为“30 天真实样本”或“算力侧独立时间外推”。完整数据可行性结论见 `docs/research/data_feasibility.md`。
