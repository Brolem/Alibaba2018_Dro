...
**尖峰（G_max）、爬坡（R_max）是约束**，不是目标，通过扫参报告可行性边界（§4.6）。**削峰不设为目标**，成本与尖峰的正常 tradeoff 由 G_max 扫参体现。
...
### 4.10 能源侧 Γ-budget（PV 残差，ε≈24.3%）

PV 相对误差 `ε` 取 2024 Ridge 验证的太阳能 `ridge_nmae ≈ 0.243`。鲁棒 PV：`pv[t] = pv_nom[t]·(1 − Γ_pv·ε)`。
...
结论：为 PV 预测误差（24.3%）留裕量，成本下降牺牲约 3.7 个百分点；与算力侧 Γ（§4.9）共同构成“双侧不确定”。

## 5. 待定

- 目标层级已定稿为“成本 → 碳”两层词典序（§4.3），但**代码尚未实现**第二层碳与 PV 自用率指标。
- 本地 PV 已按“系统太阳形状 × 场景容量”接入；2025 本地剖面仅在审稿要求或时间富余时再重拉。
- BESS 尺寸/效率/SOC 已按场景实现（效率 90%、SOC 10–90%、初始 50%、能量=2h×功率）；后续补 low/high 效率敏感性。
- 双侧 Γ-budget RO 已落地（§4.9 + §4.10）；样本外回测已完成（见 results.md §6），证明鲁棒方案降低违约率。
...
- PV 取 `1.0×P_must` 表示“白天 PV 平均能覆盖必保负荷”；更高渗透率需要 BESS 吸收爬坡（见 §4.8）。
