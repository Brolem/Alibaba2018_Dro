...
### 4.8 本地 PV（系统太阳形状 × 场景容量）

本地 PV 形状取 ERCO 系统太阳预测的归一化形状，容量按 `P_must` 比例扫参（反事实场景，非实测资产）。Jan 2025，固定 `G_max=0.8×峰值`、`R_max=0.1×峰值/h`：
...
结论：爬坡约束限制 PV 渗透率；BESS 吸收 PV 爬坡，使可行 PV 从 `0.5×P_must` 提升到 `1.5×P_must`，总成本下降最高约 42.5%。

### 4.9 Γ-budget RO（算力侧能量不确定）

不确定性：批处理总能量 `E` 因作业到达/时长不确定而波动。30 天总能量的相对不确定度 `δ≈7.9%`（由 8 天日能量 `CV=0.43`，再除以 `sqrt(30)` 估计）。

不确定集（预算不确定）：`E ∈ [E_nom, E_nom + Γ·δ·E_nom]`，`Γ∈[0,1]` 为稳健预算。

鲁棒能量守恒：`Σ batch[t] = E_nom·(1 + Γ·δ)`。

Jan 2025（`G_max=0.8×峰值`、`R_max=0.1×峰值/h`、BESS `0.5×峰值/2h`、PV `1.0×P_must`）：
