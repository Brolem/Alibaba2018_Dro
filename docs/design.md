...
Jan 2025（base 功率场景，`P_must≈2.03 MW`，`P_peak≈6.73 MW`）：

- `G_max = γ·P_peak`：γ < 0.6 不可行；γ=0.6 起可行（约 4.04 MW），γ=1.0 成本下降 45.5%。
- `R_max = δ·P_peak`/h（固定 γ=1.0）：δ < 0.05 不可行；δ=0.05 起可行（约 0.34 MW/h），δ=0.05 时成本下降 38.0%，δ≥1.0 回到 45.5%。
