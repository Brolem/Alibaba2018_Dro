...
本文档集中维护算力侧从 Alibaba v2018 记录到优化约束的完整推导，以及数据证据、反事实参数、单位转换和代码数据流。主模型只保留核心约束并引用本文；实现路线与数据可行性分别见 [design.md](../reproduction/design.md) 和 [data_feasibility.md](data_feasibility.md)。
...
当前聚合不是把所有任务压缩成一个平均负荷或30天总工作量，而是先在任务级计算 $w_i$，再按观测执行开始时刻代理聚合为逐小时释放量 $q_{d,h}^{raw}$。后续 $A_t$ 和 $D_t$ 都由这条小时释放轨迹及同一个柔性窗口 $H$ 派生。因此，在“小时粒度、统一 $H$、只描述累计工作量”的当前模型中，小时聚合改变了任务的表示方式，但保留了构造包络所需的释放时序和总工作量，不会使全部任务在窗口起点同时可执行。

聚合能够保持当前包络，需要同时满足：

- 保留每个小时的释放工作量，而不是只保留均值、峰值或总量；
- 先计算任务 core-hour 再按释放时刻求和，不用任务数直接代替工作量；
- $A_t$ 与 $D_t$ 必须由同一条聚合释放轨迹生成；
- 使用统一柔性窗口 $H$，且时间分辨率保持为1小时；
- 归一化和容量闭合必须整体作用于释放量及其上下包络。

以下操作会改变或破坏包络含义：只保存平均负荷或总工作量会丢失释放时序；把全部工作放到首小时会人为放大早期 $A_t$；改用日级聚合会隐藏日内先后关系；任务具有不同截止期却统一为一个 $H$ 会近似完成下界；独立平滑、截断或缩放 $A_t$ 与 $D_t$ 会使二者不再对应同一物理轨迹。

因此，当前实现可解释为“小时粒度、固定总工作量和统一反事实柔性窗口下，聚合批处理负荷的可调度域”，不能解释为任务级生产调度可行域的精确恢复。它没有保留任务优先级、依赖关系、异构资源请求或真实逐任务 deadline；这些限制来自数据和研究边界，而不是累计包络公式本身。
