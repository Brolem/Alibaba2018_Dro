...
先将 8 天 `batch_task` 聚合为逐日逐小时到达工作量矩阵 $q_{d,h}$。每个 30 天场景通过两日循环移动块有放回重采样产生，并统一缩放到

\[
W^*=\frac{30}{8}\sum_{d=1}^{8}\sum_{h=1}^{24}q_{d,h},
\qquad
\alpha_\omega=\frac{W^*}{\sum_t a_{\omega,t}}.
\]

缩放后的场景满足 $\sum_t\alpha_\omega a_{\omega,t}=W^*$。因此主实验只改变工作量的到达时序及其累计柔性包络，不把总工作量差异混入方法成本比较。算力侧场景定义为
...
日前决策只接触预测量。样本外回放将实际风光用于资源可用性，将实际碳强度用于排放核算；所有方法使用相同的容量、固定总工作量、柔性窗口、场景编号和随机种子。
